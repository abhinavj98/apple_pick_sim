...
| ID | Severity | Issue | Desired fix |
| -- | -------- | ----- | ----------- |
| C1 | High | Schema mismatch vs in-repo consumer: real episode has `joint_pos`, `tcp_pose_4x4`, metadata `pre_grasp`/`post_grasp`; older converter wanted `robot_joint_q`, `tcp_quat`, `apple_quat`, `step_idx=-1`, `rod_geometry`, `fruiting_base_pos`, `source_metadata.robot.control_hz`. | **Bit-1 (2026-08-07):** `real_to_batched_sysid.build_episode_metadata_from_real` now calls the settle-viewer native builders (`fruiting_params_from_pre_grasp_parquet`, `post_grasp_plan_from_metadata`), maps `joint_pos`, reads radii from `pre_grasp_geometry.parts`, and resolves `control_hz` / `episode_id` from dump metadata. Parity: `test_s00_d00_convert_matches_native_pre_post`. **Bit-2:** full `batched_sysid_v1` export + trajectory viz + FR3 smoke (`s02-d00_action.parquet`). Older zero-`action` logs remain `real-replay-action-zero` (optional `drive_fill` via `fill_actions_from_tcp_velocity`). |
...
| C5 | High | **`real-replay-action-zero`:** older `s00-d0*` episodes have an all-zero `action` column (commands not logged). Blocks open-loop FR3 twist replay. Prefer fixed logs (`s02-d00_action.parquet`). Temporary mitigation: `fill_actions_from_tcp_velocity` stamps `dataset_metadata.drive_fill`. Exporter refuses all-zero `action` unless `drive_fill` or `--allow-zero-action`. | Collector logs non-zero `action` for the **full** trajectory (not hold-only). |
| C6 | Medium | Catalog `parts.primary.length_m` can be full rod stock (e.g. 0.827 m on `s02`) while the variance fixture’s primary range is a short **support span**. Early smoke replay clamped length + used fixture `fruiting_base_pos`, which **broke pose parity** with `example_view_pre_grasp_settle` (native rebuild). | **Replay (2026-08-10):** convert → batched metadata → rebuild with episode `fruiting_base_pos` + oracle params (no fixture pose/length override). Longer-term: log support-span length at collect time / align catalog with T-junction semantics in `docs/real-world-proxy.md`. |
