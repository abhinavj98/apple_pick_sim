---
name: M3.0 §2.1 Quasi-Static Sysid
overview: Implement §2.1 (quasi-static stepped stiffness mapping) from `docs/system_identification.md` as a new `apple_pick_sim/system_id/` module plus a new `ApplePickSysIdEnv` Gymnasium environment that subclasses `ApplePickVicEnv`.
todos:
  - id: worktree
    content: Create git worktree ../apple_pick_sim-sysid on branch feature/sysid; sync submodules and uv deps
    status: pending
  - id: tests-fibonacci
    content: Write failing tests for fibonacci_hemisphere (unit norms, forward-facing, count, approx uniform)
    status: pending
  - id: impl-fibonacci
    content: Implement apple_pick_sim/system_id/fibonacci_hemisphere.py + __init__.py
    status: pending
  - id: tests-trajectory
    content: Write failing tests for QuasiStaticTrajectory (phase sequence, returns to center, hold frame count, move speed)
    status: pending
  - id: impl-trajectory
    content: Implement apple_pick_sim/system_id/quasi_static_trajectory.py + excitation_state.py
    status: pending
  - id: tests-sysid-env
    content: Write failing tests for ApplePickSysIdEnv (action space, obs keys, wrench guard, context roundtrip)
    status: pending
  - id: impl-sysid-env
    content: Implement apple_pick_gym/envs/apple_pick_sysid_env.py + register ApplePickSysId-v0
    status: pending
  - id: smoke-script
    content: Write apple_pick_sim/system_id/run_quasi_static.py smoke script
    status: pending
  - id: impl-doc
    content: Write docs/system-id-quasi-static-implementation.md
    status: pending
  - id: verify
    content: Run full test suite and smoke script; confirm green
    status: pending
isProject: false
---
...
```bash
git worktree add ../apple_pick_sim-sysid -b feature/sysid
cd ../apple_pick_sim-sysid && git submodule update --init --recursive
cd newton && uv sync --extra examples
cd .. && uv sync --extra gym --extra vic --extra dev
```

Open `../apple_pick_sim-sysid` in a new Cursor window (**File → New Window → Open Folder**) for ongoing work. Commits go on `feature/sysid`; open a PR and remove the worktree after merge.
...
Add `from apple_pick_gym.envs.apple_pick_sysid_env import ApplePickSysIdEnv` and include in `__all__`.
