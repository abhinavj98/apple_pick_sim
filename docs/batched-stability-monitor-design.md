...
class StabilityThresholds:
    max_force_n: float = 40.0        # reuse DEFAULT_STEM_FORCE_CAP_N (scene.py)
...
`apple_pick_sim/coupled_fruiting/scene.py` (`DEFAULT_STEM_FORCE_CAP_N = 40.0`,
...
5. **Threshold overrides:** `StabilityThresholds(max_force_n=10.0)` flags a force magnitude
   (e.g. 15 N) that would pass under the default `40.0` threshold.
