...
def main() -> None:
    ...
    print(f"Building scene (fix_to_apple={args.fix_to_apple}, seed={args.seed}) ...")
    ...
    print()
    print(
        f"apple→proxy : ({apple_to_proxy[0]:.4f}, {apple_to_proxy[1]:.4f}, {apple_to_proxy[2]:.4f})  "
        f"|len={norm_proxy:.4f} m"
    )
    print(
        f"apple→tcp   : ({apple_to_tcp[0]:.4f}, {apple_to_tcp[1]:.4f}, {apple_to_tcp[2]:.4f})  "
        f"|len={norm_tcp:.4f} m"
    )
    print()
