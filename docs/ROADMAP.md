...
- [M2.1] shipped: `apple_pick_gym/` — `ApplePickCoupledEnv`, real `Dict` observations (woody part poses/forces, apple position, TCP wrench/velocity); observation contract `docs/gym-observation-contract.md` (`info["obs_schema"] == "v2"`); reuse θ packing / subprocess patterns when M3.2 lands.
