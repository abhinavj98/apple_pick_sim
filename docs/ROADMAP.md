...
**Phenotype:** support-joint \(k_p\) × spur/stem **flexural** \(E\) on the 3D grid; CMA adds spur/stem **axial** \(E\) and support **T-roll** \(k_p\) (6D). Primary moduli fixed. See H5 `docs/handbook-youngs-cma.md` and `docs/superpowers/specs/2026-08-30-flexural-axial-moduli-design.md`.

**Follow-up (not M4.0 science gate):** if 6D CMA cannot soften spur axial enough vs the old `vbd_stretch_force` budget (~2 MPa effective floor vs 100 MPa box), run a targeted axial-bounds experiment before retuning fixtures.
