"""Static contract tests for the parallel Young's-modulus sys-ID gate."""
...
SCRIPT_PATH = Path("scripts/gate_youngs_modulus_sysid.sh")
...
def test_script_uses_strict_mode_repo_root_and_expected_defaults() -> None:
    ...
    assert 'REPORT_ROOT="${REPORT_ROOT:-tmp/youngs_modulus_sysid_gate_reports/${TS}}"' in script
    assert 'DATASET_PREFIX="${DATASET_PREFIX:-tmp/youngs_modulus_sysid_gate}"' in script
...
def test_script_runs_all_seed_stages_with_seed_specific_artifacts() -> None:
    ...
    assert "example_youngs_modulus_sys_id.py" in script
    assert '--output "${RANKING_OUTPUT}"' in script
    assert "--include-gt-candidate" in script
    assert "--use-median" in script
    assert "--hold-id-onehot" in script
    assert "--pool-directions" in script
    assert 'local RANKING_JSON="${RANKING_OUTPUT}/ranking.json"' in script

    assert "-m apple_pick_gym.batched_envs.youngs_modulus_gate_report" in script
    assert '--ranking-json "${RANKING_JSON}"' in script
...
def test_script_retries_each_gpu_stage_but_not_reporting() -> None:
    ...
    assert 'retry_step "replay seed=${SEED}" run_replay' in script
...
def test_stale_ranking_is_removed_before_any_seed_stage_executes() -> None:
    script = _script()

    run_seed_pos = script.index("run_seed() {")
    ranking_path_pos = script.index(
        'local RANKING_JSON="${RANKING_OUTPUT}/ranking.json"', run_seed_pos
    )
    invalidate_pos = script.index(
        'rm -f -- "${RANKING_JSON}" "${SEED_SUMMARY}"', ranking_path_pos
    )
    ...
    replay_execution_pos = script.index(
        'retry_step "replay seed=${SEED}" run_replay', invalidate_pos
    )
    ...
    assert (
        ...
        < replay_execution_pos
    )
    assert script.count('rm -f -- "${RANKING_JSON}" "${SEED_SUMMARY}"') == 1
    assert 'rm -rf "${RANKING_OUTPUT}"' not in script
