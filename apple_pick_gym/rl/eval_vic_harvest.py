...
def evaluate(wrapper, policy, *, episodes: int) -> dict:
    ...
    for label, key in (
        ("safety trip target force N (median)", "safety_trip_target_force_n_median"),
        ("safety trip prev target force N (median)", "safety_trip_prev_target_force_n_median"),
        ("safety trip target force jump > 5x (frac of trips)", "safety_trip_jump_5x_frac"),
    ):
        vals = [e[f"Episode / {label}"] for e in per_episode if np.isfinite(e.get(f"Episode / {label}", np.nan))]
        out[key] = float(np.mean(vals)) if vals else float("nan")
